...
function evaluate_policy(Rs, next_state, vigor_cost, policy, lag)
    ...
    # adjust this so that there's some probability of self transition (which corresponds to the task...)
    # so the travel action just has some probability of self transition...
    # the harvest action is slightly trickier, because they shouldn't get any reward when moves fail... 
