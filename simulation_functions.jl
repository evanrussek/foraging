...
function sim_forage_no_learn(param_dict)
    ...
    harvest_cost = param_dict["harvest_cost"];
    travel_cost_easy = param_dict["travel_cost_easy"];
    travel_cost_hard = param_dict["travel_cost_hard"];
...
function sim_forage_learn_mult(param_dict)
    ...
    lr_R_hat_pre_lag = param_dict["lr_R_hat_pre_lag"];
    lr_R_hat_pre_choice = param_dict["lr_R_hat_pre_choice"];

    harvest_cost_lag = param_dict["harvest_cost_lag"]*10;
    travel_cost_easy_lag = param_dict["travel_cost_easy_lag"]*10;
    travel_cost_hard_lag = param_dict["travel_cost_hard_lag"]*10;

    harvest_cost_choice = param_dict["harvest_cost_choice"]*10;
    travel_cost_easy_choice = param_dict["travel_cost_easy_choice"]*10;
    travel_cost_hard_choice = param_dict["travel_cost_hard_choice"]*10;

    # constants...
    reward_scale = 1.
    R_hat_start_lag = param_dict["R_hat_start_lag"];# reward rate for choice... # is this good?
    R_hat_start_choice = param_dict["R_hat_start_choice"];# reward rate for lag... # good?
    lr_harvest_lag_hat = .2; # set this arbitrarily # only relevant for choice...
    ...
    for trial_idx in 1:6
        ...
        reward_true = Float64[];
        time_units = Float64[];
        R_hat_choice_hist = Float64[];
        R_hat_lag_hist = Float64[];
        round_num = Int64[];
        harvest_lag_hat_hist = Float64[];
        ...
        while current_time_unit < total_time_units
            ...
            # if we're at the tree then make a choice...
            if steps_to_tree == 0

                # harvest cost choice?
                E_next_reward_harvest = last_reward_obs*decay*harvest_success_prob - vigor_cost(harvest_cost_choice, harvest_lag_hat);
                ...
                if choose_Harvest
                    ...
                    this_lag = sample_lag(lag_beta, R_hat_lag, harvest_cost_lag, lag_range);
                    harvest_lag_hat = (1 - lr_harvest_lag_hat)*harvest_lag_hat + lr_harvest_lag_hat*this_lag;

                    this_cost_lag = vigor_cost(harvest_cost_lag, this_lag);
                    this_cost_choice = vigor_cost(harvest_cost_choice, this_lag); # assume these are the same?
                    ...
                    this_phase = "HARVEST";
                    # make a lag here, but overall, don't add this lag to the likelihood
                    this_lag = sample_lag(lag_beta, R_hat_lag, travel_cost_lag, lag_range)

                    steps_to_tree = n_travel_steps;
                    this_reward = 0.

                    this_cost_lag = vigor_cost(travel_cost_lag,this_lag);
                    this_cost_choice = vigor_cost(travel_cost_choice,this_lag);
               ...
               # break
                ...
                this_lag = sample_lag(lag_beta, R_hat_lag, travel_cost_lag, lag_range)
                this_cost_lag = vigor_cost(travel_cost_lag, this_lag); #
                this_cost_choice = vigor_cost(travel_cost_choice, this_lag); # does this make sense?
            ...
            push!(R_hat_choice_hist, R_hat_choice)
            push!(R_hat_lag_hist, R_hat_lag)
        ...
        trial_df = DataFrame(lag = lags,
                ...
                reward_obs = reward_obs,
                time = time_units,
                R_hat_choice = R_hat_choice_hist,
                R_hat_lag = R_hat_lag_hist,
                round = round_num,
                reward_true = reward_true);
        ...
        trial_df[!,:subjectID] .= simID;
        trial_df[!,:lag_scale] = trial_df[!,:lag] ./ time_scale;
        trial_df[!,:trial_time_elapsed] = trial_df[!,:time].*100; # put this in scale of real data...
        trial_df[!,:trial_time_sec] = trial_df[!,:time] ./ 10
...
#### simulate single, but with lag reward rate sensitivity and harvest bias...
function sim_forage_learn_singleR_max(param_dict)
    ...
    # spread parameters
    choice_beta = param_dict["choice_beta"];  # choice_beta
    lag_beta_opp = param_dict["lag_beta_opp"]; # lag beta...
    lag_beta = param_dict["lag_beta"]; # lag beta...


    # learning rate parameters
    lr_R_hat_pre_lag = param_dict["lr_R_hat_pre"];
    lr_R_hat_pre_choice = lr_R_hat_pre_lag;

    harvest_cost_lag = param_dict["harvest_cost"]*10;
    travel_cost_easy_lag = param_dict["travel_cost_easy"]*10;
    travel_cost_hard_lag = param_dict["travel_cost_hard"]*10;
    ...
    # constants...
    reward_scale = 1.
    R_hat_start_lag = param_dict["R_hat_start"];# reward rate for choice... # is this good?
    R_hat_start_choice = R_hat_start_lag
        ...
        while current_time_unit < total_time_units
            ...
            if steps_to_tree == 0
                ...
                if choose_Harvest
                    ...
                    this_lag = sample_lag(lag_beta, lag_beta_opp*R_hat_lag, harvest_cost_lag, lag_range);
                    harvest_lag_hat = (1 - lr_harvest_lag_hat)*harvest_lag_hat + lr_harvest_lag_hat*this_lag;
                    ...
                    this_phase = "HARVEST";
                    # make a lag here, but overall, don't add this lag to the likelihood
                    this_lag = sample_lag(lag_beta, lag_beta_opp*R_hat_lag, travel_cost_lag, lag_range)
